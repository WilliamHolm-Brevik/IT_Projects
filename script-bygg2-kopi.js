...
console.log(komponent[1][0][0])


window['selKomponent' + j].addEventListener("change", function() {
// Create select options for komponent1 and komponent2
for (var j = 1; j <= 2; j++) {
  for (var i = 1; i < 3; i++) {
  ...
  window['selKomponent' + j] = document.getElementById("selKomponent" + j);
    var valgt =  window['selKomponent' + j].value;
    totalPris += window['pris' + j];
    document.getElementById("totalpris").innerHTML = totalPris;
    
    window['navn' + j] = (komponent)[valgt][0];
    window['pris' + j] = (komponent)[valgt][1];
    window['bilde' + j] = (komponent)[valgt][3];
    navnArray.push(window['navn' + j]);
    prisArray.push(window['pris' + j]);
    
    // Update the image and price based on the selected option
    if (valgt > 0) {
        document.getElementById("pris" + j).innerHTML = window['pris' + j];
        document.getElementById("bildediv" + j).innerHTML = window['bilde' + j];
    } 
    else {
        document.getElementById("pris" + j).innerHTML = "";
        document.getElementById("bildediv" + j).innerHTML = "";
    }
}  });
