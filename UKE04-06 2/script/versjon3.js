...
//gjør vartiablene globale da de skalø brukes i til funksjoner
var navn = "";
var losningsord = ""
var svaret = "";
...
//array for select og fyller select

var kategorier = ["Hovedsteder", "Land", "Premier Leauge lag"]

for(var i = 0; i<kategorier.length; i++)
{
    var valg = document.createElement("option");
    valg.innerHTML = kategorier[i];
    valg.value = i;
    document.getElementById("selKategori").appendChild(valg);
}
...
function reg()
{
    ...
    var alternativer = []
    alternativer[0] = ["LONDON", "HELSINKI", "TOKYO", "BEIJING", "MADRID", "MOSKVA", "OSLO", "WIEN"];
    alternativer[1] = ["NORGE", "POLEN", "TYSKLAND", "SAUDI ARABIA", "SUDAN", "YEMEN", "USA", "INDONESIA", "AUSTRALIA"];
    alternativer[2] = ["CHELSEA", "ARSENAL", "TOTTENHAM", "EVERTON", "LEEDS", "LIVERPOOL", "NEWCASTLE", "BRIGHTON"];

    //henter inn riktig kategori fra select

    var valgt = document.getElementById("selKategori").value;


    losningsord = alternativer[valgt][Math.floor(Math.random()*alternativer[valgt].length)]
    ...
    //skriver ut valgt kategori til bruker
    document.getElementById("info").innerHTML = "Din kategori er: " + kategorier[valgt];
    ...
    //lukker modalen 
    document.getElementById("minModal").style.display = "none"

}

//bruker enter isteden for testknapp

//lager variabel av input

var input = document.getElementById("txtBokstav");

//lager en funsjon som sjkal startes med at man klikker på en bestemt  knapp i tekstfelt (variablene)
